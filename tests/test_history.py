...
from kubeproof.history import (
    FilesystemBundleStore,
    HistoryError,
    HistoryService,
    SQLiteEvaluationIndex,
)
from kubeproof.web import APP_HTML
...
def test_embedded_ui_exposes_history_and_check_drill_down() -> None:
    assert "KubeProof History" in APP_HTML
    assert "/api/evaluations" in APP_HTML
    assert "check.observation_ids" in APP_HTML
